...
class UjianController extends Controller
{
    ...
    public function index()
    {
        ...
        // Ambil ujian berdasarkan role user
        if (auth()->user()->roles_id == 1 || auth()->user()->roles_id == 2) {
            // Admin dan Pengajar - tampilkan semua ujian
            $ujian = Ujian::with(['kelasMapel.mapel', 'kelasMapel.kelas'])->get();
        } else {
            // Siswa - tampilkan ujian dari kelas mereka
            $ujian = Ujian::whereHas('kelasMapel.kelas', function($query) {
                $query->where('id', auth()->user()->kelas_id);
            })->with(['kelasMapel.mapel', 'kelasMapel.kelas'])->get();
        }

        return view('menu.ujian.index', [
