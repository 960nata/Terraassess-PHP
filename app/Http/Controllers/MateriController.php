...
class MateriController extends Controller
{
    ...
    public function viewMateri(Request $request)
    {
        ...
        if (!$request->has('token') || !$request->has('kelasMapelId') || !$request->has('mapelId')) {
            // If parameters are missing, redirect to teacher dashboard with error message
            return redirect()->route('teacher.dashboard')->with('error', 'Parameter yang diperlukan tidak ditemukan. Silakan akses materi melalui menu kelas.');
        }
        ...
        try {
            ...
            if (!$materi) {
                return redirect()->route('teacher.dashboard')->with('error', 'Materi tidak ditemukan.');
            }
            ...
            if (!$kelasMapel) {
                return redirect()->route('teacher.dashboard')->with('error', 'Data kelas mapel tidak ditemukan.');
            }
            ...
            return redirect()->route('teacher.dashboard')->with('error', 'Token tidak valid. Silakan akses materi melalui menu kelas.');
        } catch (\Exception $e) {
            // Handle other errors
            return redirect()->route('teacher.dashboard')->with('error', 'Terjadi kesalahan saat memuat materi.');
        }
    }
