        ...
        <div class="relative w-full max-w-md modal-glass rounded-2xl shadow-2xl transform scale-95 transition-all duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <form class="px-4 sm:px-8 pb-6 sm:pb-8" method="POST" action="{{ route('authenticate') }}" id="loginForm">
                ...
                <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm">
                    <a href="{{ route('forgotPassword') }}" class="text-white/60 hover:text-white transition-colors flex items-center gap-2">
                    ...
                    <div class="flex items-center gap-2 text-white/40">
                        ...
                        <a href="{{ route('register') }}" class="text-purple-400 hover:text-purple-300 font-bold transition-colors underline decoration-purple-500/30 underline-offset-4">Daftar Sekarang</a>
                    </div>
                </div>

                <!-- Error Messages -->
                @if(isset($hasAdmin) && $hasAdmin == 0)
                    <div class="mt-8 p-4 bg-yellow-500/15 border border-yellow-500/30 rounded-xl flex items-center gap-3 text-yellow-300 text-sm">
                        <i class="fas fa-exclamation-triangle text-yellow-400"></i>
                        <div>
                            Akun <strong>Admin</strong> belum dibuat, 
                            <a href="{{ route('adminRegister') }}" class="underline hover:no-underline font-bold text-yellow-200">Buat sekarang</a>
                        </div>
                    </div>
                @endif
