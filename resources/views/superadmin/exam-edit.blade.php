...
<div class="container-fluid">
    ...
    <div class="page-header mb-6">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('superadmin.exam-management') }}" class="btn btn-outline">
    ...
    <form id="examForm" method="POST" action="{{ route('superadmin.exam-management.update', $exam->id) }}">
        @csrf
        @method('PUT')
            ...
            <div class="lg:col-span-2">
                <div class="space-y-6">
                        ...
                        <div class="card-body space-y-4">
                            <div>
                                ...
                                <input type="text" name="exam_title" class="form-input" 
                                       value="{{ old('exam_title', $exam->name) }}" required>
                                @error('exam_title')
                            ...
                            <div>
                                ...
                                <textarea name="exam_description" class="form-textarea" rows="4" 
                                          placeholder="Tuliskan instruksi yang jelas untuk siswa..." required>{{ old('exam_description', $exam->content) }}</textarea>
                                    ...
                                    <select name="class_id" class="form-select" required>
                                        ...
                                        @foreach($classes as $class)
                                            <option value="{{ $class->id }}" 
                                                    {{ old('class_id', $exam->KelasMapel->Kelas->id) == $class->id ? 'selected' : '' }}>
                                    ...
                                    <select name="subject_id" class="form-select" required>
                                        ...
                                        @foreach($subjects as $subject)
                                            <option value="{{ $subject->id }}" 
                                                    {{ old('subject_id', $exam->KelasMapel->Mapel->id) == $subject->id ? 'selected' : '' }}>
                                                {{ $subject->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="duration" class="form-input" 
                                           value="{{ old('duration', $exam->time) }}" min="1" max="300" required>
                                    @error('duration')
                                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="due_date" class="form-input" 
                                       value="{{ old('due_date', $exam->due ? $exam->due->format('Y-m-d\TH:i') : '') }}" required>
                                @error('due_date')
                                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select name="exam_type" class="form-select" required>
                                    ...
                                    <option value="multiple_choice" {{ old('exam_type', $exam->tipe == 1 ? 'multiple_choice' : '') == 'multiple_choice' ? 'selected' : '' }}>
                                        Pilihan Ganda
                                    </option>
                                    <option value="essay" {{ old('exam_type', $exam->tipe == 2 ? 'essay' : '') == 'essay' ? 'selected' : '' }}>
                                        Essay
                                    </option>
                                    <option value="mixed" {{ old('exam_type', $exam->tipe == 3 ? 'mixed' : '') == 'mixed' ? 'selected' : '' }}>
                                        Campuran
                                    </option>
                                </select>
                                @error('exam_type')
                            ...
                            <div class="flex items-center space-x-4">
                                <label class="flex items-center">
                                    <input type="checkbox" name="is_hidden" value="1" 
                                           {{ old('is_hidden', $exam->isHidden) ? 'checked' : '' }} 
                                           class="form-checkbox">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            @if($exam->tipe == 1 || $exam->tipe == 3)
                                <!-- Multiple Choice Questions -->
                                <div class="space-y-4">
                                    <h4 class="text-lg font-medium text-white">Soal Pilihan Ganda</h4>
                                    @php
                                        $multipleQuestions = $exam->soalUjianMultiple;
                                    @endphp
                            ...
                            @if($exam->tipe == 2 || $exam->tipe == 3)
                                <!-- Essay Questions -->
                                <div class="space-y-4 mt-6">
                                    <h4 class="text-lg font-medium text-white">Soal Essay</h4>
                                    @php
                                        $essayQuestions = $exam->soalUjianEssay;
                                    @endphp
            ...
            <div class="space-y-6">
                ...
                <div class="card">
                    ...
                    <div class="card-body space-y-3">
                        ...
                        <a href="{{ route('superadmin.exam-management.view', $exam->id) }}" class="btn btn-outline w-full">
                            <i class="ph-eye mr-2"></i>
                            Lihat Detail
                        </a>
                        
                        <a href="{{ route('superadmin.exam-management') }}" class="btn btn-outline w-full">
                            <i class="ph-arrow-left mr-2"></i>
                            Kembali ke Daftar
                        </a>
                    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body space-y-3">
                        <div>
                            ...
                            <div class="text-white font-mono">{{ $exam->id }}</div>
                        </div>
                        
                        <div>
                            <label class="text-sm text-gray-400">Dibuat</label>
                            <div class="text-white">{{ $exam->created_at->format('d M Y, H:i') }}</div>
                        </div>
                        
                        <div>
                            <label class="text-sm text-gray-400">Terakhir Diupdate</label>
                            <div class="text-white">{{ $exam->updated_at->format('d M Y, H:i') }}</div>
                        </div>
