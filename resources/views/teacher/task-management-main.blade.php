...
.task-filters {
        ...
        .tasks-table-content td {
            padding: 1rem;
            border-bottom: 1px solid #334155;
        }

        .tasks-table-content tr:hover {
            background: #334155;
        }
...
<div class="task-type-cards">
    <div class="task-type-card" onclick="createMultipleChoiceTask()">
        <div class="task-card-header">
            ...
            <div class="task-card-stats">
                <span class="task-count">{{ $stats['multiple_choice'] ?? 0 }}</span>
    ...
    <div class="task-type-card essay" onclick="createEssayTask()">
        <div class="task-card-header">
            ...
            <div class="task-card-stats">
                <span class="task-count">{{ $stats['essay'] ?? 0 }}</span>
                <span class="task-label">Tugas</span>
            </div>
        </div>
        <div class="task-card-content">
    ...
    <div class="task-type-card individual" onclick="createIndividualTask()">
        <div class="task-card-header">
            ...
            <div class="task-card-stats">
                <span class="task-count">{{ $stats['individual'] ?? 0 }}</span>
                <span class="task-label">Tugas</span>
            </div>
        </div>
        <div class="task-card-content">
    ...
    <div class="task-type-card group" onclick="createGroupTask()">
        <div class="task-card-header">
            ...
            <div class="task-card-stats">
                <span class="task-count">{{ $stats['group'] ?? 0 }}</span>
                <span class="task-label">Tugas</span>
            </div>
        </div>
        <div class="task-card-content">
...
<div class="task-filters">
    <form action="{{ route('teacher.tasks.management') }}" method="GET" class="filter-form">
        <div class="filter-row">
            ...
            <div class="form-group">
                <label for="filter_difficulty">Filter Tingkat Kesulitan</label>
...
<div class="tasks-table">
    ...
    <div class="table-responsive">
        <table class="tasks-table-content">
            <thead>
                <tr>
                    <th>Nama Tugas</th>
                    <th>Kelas</th>
                ...
                <tr>
                    <td>
                        <div class="task-name">
                            ...
                            @if($task->content)
                                <p class="task-description">{{ Str::limit($task->content, 50) }}</p>
                            @endif
                        </div>
                    </td>
                    ...
                    <td>
                        @php
                            $tipeMap = [1 => 'Pilihan Ganda', 2 => 'Essay', 3 => 'Mandiri', 4 => 'Kelompok'];
                        @endphp
                        <span class="task-type-badge">{{ $tipeMap[$task->tipe] ?? 'N/A' }}</span>
                    </td>
                    <td>
                        @php
                    ...
                    <td>
                        <div class="action-buttons">
                            <button class="btn-secondary" onclick="editTask({{ $task->id }})" title="Edit">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn-success" onclick="viewSubmissions({{ $task->id }})" title="Lihat">
                                <i class="fas fa-eye"></i>
                            </button>
                            @if($task->isHidden)
                                <button class="btn-primary" onclick="publishTask({{ $task->id }})" title="Publikasi">
                                    <i class="fas fa-eye"></i>
                                </button>
                            @else
                                <button class="btn-warning" onclick="unpublishTask({{ $task->id }})" title="Sembunyikan">
                                    <i class="fas fa-eye-slash"></i>
                                </button>
                            @endif
                            <button class="btn-danger" onclick="deleteTask({{ $task->id }})" title="Hapus">
...
<div class="tasks-table">
    <h2 style="color: #ffffff; margin-bottom: 1.5rem; font-size: 1.25rem;">
        <i class="fas fa-list me-2"></i>Tugas Terbaru
    </h2>
