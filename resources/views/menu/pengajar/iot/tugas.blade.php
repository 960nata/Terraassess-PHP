...
@section('container')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="text-white display-4 fw-bold mb-2">
                <i class="fas fa-microchip me-3 text-primary"></i>Tugas IoT
            </h1>
            <p class="text-white-75 fs-5 mb-0">Kelola dan pantau tugas IoT untuk siswa</p>
        </div>
        <div class="text-end">
    ...
    <div class="row mb-4">
        <div class="col-md-3 col-6 mb-3">
            <div class="glass-card p-4 text-center">
                ...
                <h2 class="text-white mb-1" id="totalReadings">0</h2>
                <small class="text-white-75">Total Data</small>
            </div>
        </div>
        <div class="col-md-3 col-6 mb-3">
            <div class="glass-card p-4 text-center">
                <i class="fas fa-calendar-day fa-3x text-success mb-3"></i>
                <h2 class="text-white mb-1" id="todayReadings">0</h2>
        ...
        <div class="col-md-3 col-6 mb-3">
            <div class="glass-card p-4 text-center">
                ...
                <h2 class="text-white mb-1" id="avgTemperature">0°C</h2>
        ...
        <div class="col-md-3 col-6 mb-3">
            <div class="glass-card p-4 text-center">
                ...
                <h2 class="text-white mb-1" id="avgMoisture">0%</h2>
        ...
        <div class="col-12">
            <div class="glass-card p-4">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-4">
                            <label class="text-white-75 fw-semibold mb-2">Pilih Kelas</label>
                        ...
                        <div class="mb-4">
                            <label class="text-white-75 fw-semibold mb-2">Pilih Siswa</label>
                            <select class="form-control" id="selectSiswa" disabled>
                ...
                <div class="text-center">
                    <button class="btn btn-primary btn-lg me-3" id="scanDeviceBtn" onclick="scanDevice()">
                        <i class="fas fa-bluetooth me-2"></i>Scan & Test Alat
                    </button>
            ...
            <div class="glass-card p-4">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h3 class="text-white mb-0">
                        <i class="fas fa-table me-2 text-primary"></i>Data Terbaru
                    </h3>
                    <div>
    ...
    <script>
        ...
        async function loadStatistics() {
            try {
                const response = await fetch('/api/iot/readings/statistics');
                const result = await response.json();
                
                if (result.success) {
                    document.getElementById('totalReadings').textContent = result.data.total_readings;
                    document.getElementById('todayReadings').textContent = result.data.today_readings;
                    document.getElementById('avgTemperature').textContent = result.data.average_temperature + '°C';
                    document.getElementById('avgMoisture').textContent = result.data.average_moisture + '%';
                }
            } catch (error) {
        ...
        document.getElementById('selectKelas').addEventListener('change', async function() {
            ...
            const studentSelect = document.getElementById('selectSiswa');
            
            if (classId) {
                try {
                    const response = await fetch(`/api/iot/readings/class/${classId}`);
                    const result = await response.json();
                    
                    studentSelect.innerHTML = '<option value="">-- Pilih Siswa --</option>';
                    
                    if (result.success && result.data.data) {
                        const students = [...new Set(result.data.data.map(reading => ({
                            id: reading.student_id,
                            name: reading.student ? reading.student.name : reading.student_id
                        })))];
        ...
        async function scanDevice() {
            ...
            try {
                ...
                scanBtn.innerHTML = '<i class="fas fa-bluetooth me-2"></i>Scan & Test Alat';
            }
        }
            ...
            try {
                const response = await fetch('/api/iot/readings', {
                    ...
                    body: JSON.stringify({
                        ...
                        class_id: classId,
                        soil_temperature: parseFloat(currentData.soil_temperature),
                        soil_humus: parseFloat(currentData.soil_humus),
                        soil_moisture: parseFloat(currentData.soil_moisture),
        ...
        function exportData() {
            const classId = document.getElementById('selectKelas').value;
            let url = '/api/iot/readings/export';
